...
export function registerPlanRoutes(
  app: FastifyInstance,
  deps: { authorizer: RequestAuthorizer; planRepository: PlanRepository },
): void {
  ...
  app.get('/api/v1/plans', async (request, reply) => {
    ...
    if (!parsed.success) {
      return sendValidationError(reply, request, 'Invalid plan date range', parsed.error.flatten());
    }
  ...
  app.post('/api/v1/plans', async (request, reply) => {
    await requireScopes(request, authorizer, ['plans:write']);
    const parsed = createPlanSchema.safeParse(request.body);
    if (!parsed.success) {
      return sendValidationError(reply, request, 'Invalid plan item', parsed.error.flatten());
    }
    ...
    return reply.status(201).send(serializePlan(created));
  });
  ...
  app.post('/api/v1/plans/:id/progress', async (request, reply) => {
    ...
    if (!parsed.success) {
      return sendValidationError(reply, request, 'Invalid progress value', parsed.error.flatten());
    }

    const plan = await planRepository.findById(id);
    if (!plan) {
      return reply.status(404).send(errorBody(request, 'not_found', 'Plan item not found'));
    }
